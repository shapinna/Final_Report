...
ggplot(gap,aes(x=life))+geom_histogram()+facet_grid(~region)


library('lattice')
histogram(~ life | region, data = gap)


histogram(~region, data=gap, 
      type = "density",layout=c(1,2),
      panel=function(x, ...) {
        panel.histogram(x, ...)
        panel.mathdensity(dmath=dnorm, col="black",
# Add na.rm = TRUE to mean() and sd()
                          args=list(mean=mean(gap$life, na.rm = TRUE),
                                    sd=sd(gap$life, na.rm = TRUE)), ...)})
...
- since slope is positive we can assume these variables are have positive relationship
