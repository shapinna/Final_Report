...
```{r}
gap_d <- read.csv("https://raw.githubusercontent.com/shapinna/Final_Report/master/data/gapminder.csv", header=TRUE)
...
* No of records for each region
  - America region has 7961 records 
  - East Asia & Pacific has 6256 records
  - Europe & Central Asia has 10468 records
  - Middle East & North Africa has 4309 records
  - Sub-Saharan Africa has 10562 records
  - South Asia has 1728 records 


# Questions

* Which region is better in Life expectancy and Income before 1950 & after 1950?
* Whether life expectancy increased based on Income?
...
* Croatia Income data has been collected only from 1820 and prior years is not collected hence it is not logical to populate missing income values of earlier years with mean or median.
...
* south Asia is poor peformer in income after 1950 with low mean and median income
...
### Correlation between elements of dataset
...
hist(gap$life,
     col="grey75",
     main="Distribution of Life across all regions",
     xlab ="Life",
     prob= TRUE)
...
### Linear regression model with life as dependent on Income
...
* Intercept $\alpha$ 38.73 indicates intercept value while all other values are zero in a linear regression model
  - P value <2e-16 < 0.05 indicates that it is statistically singnificant at $\alpha$ 0.05 level
  
*	$\beta$ (b in sample prediction equation) represents the slope in a linear regression model  
* Income coefficient estimate 0.0009217 indicates
  - by holding all other variables constant, a unit increase in income will increase life by 0.0009217 units
  - $\beta$>0 indicates both Life and Income are positively related
  - test statistic t = 139.2 & P value <2e-16 < 0.05 indicates that it is statistically singnificant at $\alpha$ 0.05 level and we reject null hypothesis of $\beta$ =0

* $\beta$ is not equal to 0  represents Life and Income are not  independent
* As R squared = 0.3322 denotes variation in Life can be explained to the extent of 33.22% by variation in Income

* 	P value of F test< 2.2e-160 < 0.05 we reject null hypothesis of βi =0, indicates multiple regression model as a whole is explaining the variation in dependent variable sales
...
* Adjusted R square 33.22% represent variance in life is explained by variance in income by 33.22%
...
* QQ Plot indicates residuals are not normally distributed

Since the linear regression model has heteroscedasticity and residuals vs pridicted are not normally distributed,  we can conclude linear regression model does not fit variables between Life and Income.




### Adding Interaction term to linear regression model on life dependent on income

```{r}

gap_lm_int <- lm(life~Year+income*as.factor(region),data=gap) # Linear regression model
...
* R has choosen America region as the reference category

* $\alpha$ Intercept -2.722e+02 indicates y intercept when all other values are zero including region

* Coefficient estimate $\beta$ for the year 1.639e-01 indicates
  - By holding all other variables constant a unit increase in Year will increase life by 1.639e-01 units
  - p value < 2e-16 < 0.05 hence we reject null hyposthesis of $\beta$ = 0 it represents variation in life is explained by variation in year
  
* coefficeient estimate income 9.140e-04 denotes value for reference category America
  - By holding all other variables constant for a reference category America for every unit increase in income will increase life by 9.140e-04 units
  - P value < 2e-16 < 0.05 indicates variation in life is explained by variation in income for a reference category region = America by holding all other variables constant

* coefficient as.factor(region)East Asia & Pacific -5.906e-01 denotes difference in intercept  between reference category region = America and region =East Asia & Pacific, while all other variables are zero including income
  - Since intercept value for region America = -2.722e+02   hence intercept value for region East Asia & Pacific = -2.722e+02 -5.906e-01 = -272.7906
  - P value 0.00043 < .05 indicates t statics is significant at $\alpha$ 0.05 level
  
* coefficient as.factor(region)Europe & Central Asia 3.990e+00   denotes difference in intercept  between reference category region = America and region =Europe & Central Asia, while all other variables are zero including income
  - Since intercept value for region America = -2.722e+02   hence intercept value for region Europe & Central Asia = -2.722e+02 + 3.990e+00   = -268.21
  - P value < 2e-16  < .05 indicates t statics is significant at $\alpha$ 0.05 level

* coefficient as.factor(region)Middle East & North Africa -9.524e-01   denotes difference in intercept  between reference category region = America and region =Middle East & North Africa, while all other variables are zero including income
  - Since intercept value for region America = -2.722e+02   hence intercept value for region Middle East & North Africa = -2.722e+02 -9.524e-01 = -273.1524
  - P value 1.58e-07  < .05 indicates t statics is significant at $\alpha$ 0.05 level
  
* coefficient as.factor(region)South Asia -7.609e+00   denotes difference in intercept  between reference category region = America and region =South Asia, while all other variables are zero including income
  - Since intercept value for region America = -2.722e+02   hence intercept value for region South Asia = -2.722e+02 -7.609e+00 = -279.809
  - P value < 2e-16  < .05 indicates t statics is significant at $\alpha$ 0.05 level

* coefficient as.factor(region)Sub-Saharan Africa -3.843e+00  denotes difference in intercept  between reference category region = America and region =Sub-Saharan Africa, while all other variables are zero including income
  - Since intercept value for region America = -2.722e+02   hence intercept value for region Sub-Saharan Africa = -2.722e+02 -3.843e+00 = -276.043
  - P value < 2e-16  < .05 indicates t statics is significant at $\alpha$ 0.05 level


* Coefficient income:as.factor(region)East Asia & Pacific -4.265e-04  denotes the slope difference between income for the reference category = America & East Asia & Pacific, as the slope for reference cateogry is 9.140e-04  slope for income for East Asia & Pacific = 9.140e-04-4.265e-04 = 0.0004875
  - P value < 2e-16 <0.05 indicates variation in life is explained by variation in income where region is East Asia & Pacific

* Coefficient income:as.factor(region)Europe & Central Asia -3.204e-04 denotes the slope difference between income for the reference category = America & Europe & Central Asia, as the slope for reference cateogry is 9.140e-04  slope for income for Europe & Central Asia = 9.140e-04 -3.204e-04 = 
  - P value < 2e-16 <0.05 indicates variation in life is explained by variation in income where region is Europe & Central Asia

* Coefficient income:as.factor(region)Middle East & North Africa -6.531e-04  denotes the slope difference between income for the reference category = America & Middle East & North Africa, as the slope for reference cateogry is 9.140e-04  slope for income for Middle East & North Africa = 9.140e-04 -6.531e-04 =
  - P value < 2e-16 <0.05 indicates variation in life is explained by variation in income where region is Middle East & North Africa
  
* Coefficient income:as.factor(region)South Asia 2.332e-03 denotes the slope difference between income for the reference category = America & South Asia, as the slope for reference cateogry is 9.140e-04  slope for income for South Asia = 9.140e-04+2.332e-03 = 0.0004875
  - P value < 2e-16 <0.05 indicates variation in life is explained by variation in income where region is South Asia
  
* Coefficient income:as.factor(region)Sub-Saharan Africa -2.071e-04  denotes the slope difference between income for the reference category = Sub-Saharan Africa, as the slope for reference cateogry is 9.140e-04  slope for income for Sub-Saharan Africa = 9.140e-04 -2.071e-04  = 
  - P value 1.08e-08  <0.05 indicates variation in life is explained by variation in income where region is Sub-Saharan Africa
  
* 	P value of F test< 2.2e-160 < 0.05 we reject null hypothesis of βi =0, indicates multiple regression model as a whole is explaining the variation in dependent variable sales

* degrees of freedom (n-13) 38930 indicates that only 38943 records are considered due to missing income values of 2341 records out of 41284 observations

 
#### Residuals to be normally distributed
 
```{r}
hist(gap_lm_int$residuals,
     col="grey75",
     main="Distribution of residuals",
     xlab= "Residuals",
     prob= TRUE)
curve(dnorm(x,mean(gap_lm_int$residuals),
            sd(gap_lm_int$residuals)),
      add=TRUE,
      col="red",
      lwd=3)
...
* As per above plot there is a slight deviation in the normal distribution but the data is well with in the normal distribution curve.
* we can conclude residuals are normally distributed for a given year and region

#### QQ plot

```{r}
plot(gap_lm_int,which = 2)
```

* As per above QQ plot there is a slight deviation of residuals
* over all residuals are normally distribution for the interaction term linear regression model


#### Verify the variability in the residuals is same for both large and small values for estimated Y

```{r}
plot(gap_lm_int$residuals~gap_lm_int$fitted.values,
     xlab="Fitted values",
     ylab="Residuals",
     main="Residual vs Predicted values")
abline(h=0,col="red",lty=2,lwd=2)
...
* As per above plot of fitted vs residuals variance of data for highs and lows are different it does data has heteroscedasticity
...
* Satistical test concludes interaction term linear regression model has heteroscedasticity.
* Over all interaction term linear regression model is best suits compared to normal regression model interms of heteroscedasticity


#### Analysis on whether high income will always have high life expectancy

As R squared is 0.7514,  Variation in Life (dependent variable) to the extent of 75% is explained by variation in income(indpendent variable) by controlling Year and Region

Hence we can conclude for given Year and Region high life is a result of high income 
...
* Intercept $\alpha$ -6315250 indicates intercept value while all other values are zero in a linear regression model
  - P value 2.33e-13  < 0.05 indicates that it is statistically singnificant at $\alpha$ 0.05 level
  
*	$\beta$ (b in sample prediction equation) represents the slope in a linear regression model  
* Income coefficient estimate 441077 indicates
  - by holding all other variables constant, a unit increase in Life will increase population by 441077 units
  - $\beta$>0 indicates both Life and population are positively related
  - test statistic t = 23.474 & P value <2e-16 < 0.05 indicates that it is statistically singnificant at $\alpha$ 0.05 level and we reject null hypothesis of $\beta$ =0

* $\beta$ is not equal to 0  represents Life and Population are not  independent

* Adjusted R squared = 0.01315 denotes variation in Population can be explained to the extent of 1.31% by variation in Life

* 	P value of F test< 2.2e-160 < 0.05 we reject null hypothesis of βi =0, indicates multiple regression model as a whole is explaining the variation in dependent variable sales

* degrees of freedom (n-2) 41282 indicates that all 41284 observations are considered
...
ncvTest(gap_mod2) # test of heterscedasticity
...
Hence linear regression model is not suitable for life and population variables
...
#kmeansObj <- kmeans(gap_1, centers = 4)
...
avg_sil <- function(k) {
  km.res <- kmeans(gap_1, centers = k, nstart = 25)
  ss <- silhouette(km.res$cluster, dist(gap_1))
  mean(ss[, 3])
}
...
**Not able to run above code due to memory issues**
...
Visualization of 4 clusters they are tightly grouped with 4 centroids.
 ...
 **Which region is better in Life and Income before 1950 & after 1950?**
 ...
 - Before 1950 Life
    - Europe and central asia are better performers with mean and median of 39 & 37 years with standard deviation fo 9
  ...
  - After 1950 Life
    ...
    - Europe & central asia are better performers with mean and median of 71 years with lower standard deviation of 6 years
    - South Asia has improved after 1950
    - Sub-Saharan africa is poor performer in life with mean and median of 54 and with standard deviation of 8 compared to other regions
  
  - Before 1950 Income
    - Europe & Central Asia are better performers with high mean & median income with high standard deviation
    - sub saharan africa is poor peformer in income before 1950 with low mean and median income
    
  - After 1950 Income
    -  Europe & central Asia are better performers since median income is high with a standard deviation of 13K lower than 32K compared to middle east region
    - South Asia is poor performer in income with low mean and median income

 **Whether life expectancy increases based on Income?**

  - As per above linear regression model with interaction term where life is dependent on income with year and region, adjusted R squared is 0.7514. It represents variation in life (dependent variable) to the extent of 75% is explained by variation in income(indpendent variable) by controlling year and region.
